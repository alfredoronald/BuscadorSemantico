...
// Configuración para consultas a DBpedia
const DBPEDIA_SPARQL_ENDPOINT = "https://dbpedia.org/sparql";
const CACHE_DBPEDIA = new Map(); // Caché para evitar consultas repetidas
...
function cargarOntologia() {
  ...
  const owlPathUrl =
    "file:///" + owlPath.replace(/\\/g, "/").replace(/ /g, "%20");
  ...
  try {
    ...
    // Contar cuántos enlaces owl:sameAs hay
    const sameAsCount = store.statementsMatching(
      null,
      new $rdf.NamedNode(OWL + "sameAs"),
      null,
    ).length;
    console.log(`🔗 Enlaces owl:sameAs encontrados: ${sameAsCount}`);
  } catch (err) {
...
// ============================================================
// FUNCIONES PARA CONSULTAR DBPEDIA (CORREGIDAS)
// ============================================================

function consultarDBpediaSPARQL(query) {
  return new Promise((resolve, reject) => {
    const encodedQuery = encodeURIComponent(query);
    const requestUrl = `${DBPEDIA_SPARQL_ENDPOINT}?query=${encodedQuery}&format=json`;
    ...
    const options = {
      ...
      headers: {
        ...
        "User-Agent":
          "Mozilla/5.0 (compatible; TurismoBot/1.0; +http://localhost:3000/bot)",
      },
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        // Verificar si la respuesta es HTML (error de DBpedia)
        if (data.trim().startsWith("<") && !data.trim().startsWith("<?xml")) {
          reject(
            new Error(
              "DBpedia devolvió HTML en lugar de JSON - El endpoint puede estar caído",
            ),
          );
          return;
        }
    ...
    req.on("timeout", () => {
      req.destroy();
      reject(new Error("Timeout consultando DBpedia"));
    });
    req.end();
  });
}

async function enriquecerConDBpedia(recursoURI) {
  // Verificar caché
  if (CACHE_DBPEDIA.has(recursoURI)) {
    return CACHE_DBPEDIA.get(recursoURI);
  }

  // Extraer el nombre del recurso para búsqueda alternativa
  const resourceName = recursoURI.split("/").pop().replace(/_/g, " ");

  // Consulta principal
  const query = `
      SELECT DISTINCT ?label ?abstract ?thumbnail ?lat ?long ?wikiPage
      WHERE {
        OPTIONAL { <${recursoURI}> rdfs:label ?label . FILTER(LANG(?label) = "es") }
        OPTIONAL { <${recursoURI}> dbo:abstract ?abstract . FILTER(LANG(?abstract) = "es") }
        OPTIONAL { <${recursoURI}> dbo:thumbnail ?thumbnail }
        OPTIONAL { <${recursoURI}> geo:lat ?lat }
        OPTIONAL { <${recursoURI}> geo:long ?long }
        OPTIONAL { <${recursoURI}> foaf:isPrimaryTopicOf ?wikiPage }
      }
      LIMIT 1
    `;

  try {
    const result = await consultarDBpediaSPARQL(query);
    const bindings = result?.results?.bindings?.[0] || {};

    // Si no encontramos nada, intentar búsqueda por nombre
    if (!bindings.label && !bindings.abstract && !bindings.thumbnail) {
      const searchQuery = `
          SELECT DISTINCT ?uri ?label ?abstract ?thumbnail ?lat ?long ?wikiPage
          WHERE {
            ?uri rdfs:label ?label .
            FILTER(LANG(?label) = "es")
            FILTER(CONTAINS(LCASE(?label), LCASE("${resourceName.replace(/"/g, '\\"')}")))
      ...
      const searchResult = await consultarDBpediaSPARQL(searchQuery);
      const searchBindings = searchResult?.results?.bindings?.[0] || {};

      const enriquecido = {
        dbpediaLabel: searchBindings.label?.value || null,
        dbpediaAbstract: searchBindings.abstract?.value || null,
        dbpediaThumbnail: searchBindings.thumbnail?.value || null,
        dbpediaLat: searchBindings.lat?.value
          ? parseFloat(searchBindings.lat.value)
          : null,
        dbpediaLong: searchBindings.long?.value
          ? parseFloat(searchBindings.long.value)
          : null,
        dbpediaWikiPage: searchBindings.wikiPage?.value || null,
      };

      CACHE_DBPEDIA.set(recursoURI, enriquecido);
      return enriquecido;
    }

    const enriquecido = {
      dbpediaLabel: bindings.label?.value || null,
      dbpediaAbstract: bindings.abstract?.value || null,
      dbpediaThumbnail: bindings.thumbnail?.value || null,
      dbpediaLat: bindings.lat?.value ? parseFloat(bindings.lat.value) : null,
      dbpediaLong: bindings.long?.value
        ? parseFloat(bindings.long.value)
        : null,
      dbpediaWikiPage: bindings.wikiPage?.value || null,
    };

    CACHE_DBPEDIA.set(recursoURI, enriquecido);
    return enriquecido;
  } catch (error) {
    // No mostrar error en consola para no saturar - solo log silencioso
    // console.log(`⚠️ DBpedia no disponible para ${recursoURI}`);
    CACHE_DBPEDIA.set(recursoURI, null);
    return null;
  }
}

async function enriquecerEntidadConDBpedia(entidad) {
  if (!entidad.sameAs || entidad.sameAs.length === 0) {
    return entidad;
  }

  // Buscar el primer sameAs que sea de DBpedia
  const dbpediaLink = entidad.sameAs.find((link) =>
    link.includes("dbpedia.org/resource/"),
  );
  if (!dbpediaLink) {
    return entidad;
  }

  const enriquecido = await enriquecerConDBpedia(dbpediaLink);
  if (
    enriquecido &&
    (enriquecido.dbpediaLabel ||
      enriquecido.dbpediaAbstract ||
      enriquecido.dbpediaThumbnail)
  ) {
    return {
      ...entidad,
      dbpedia: enriquecido,
...
async function enriquecerListaConDBpedia(entidades) {
  const enriquecidas = [];
  for (const entidad of entidades) {
    try {
      const enriquecida = await enriquecerEntidadConDBpedia(entidad);
      enriquecidas.push(enriquecida);
    } catch (error) {
      // Si falla, devolver la entidad sin enriquecer
      enriquecidas.push(entidad);
    }
  }
  return enriquecidas;
}
...
// ============================================================
// FUNCIONES DE NORMALIZACIÓN Y DICCIONARIO
// ============================================================
...
const DICCIONARIO = {
  ...
  gratis: "gratuito",
  "sin costo": "gratuito",
  "entrada gratis": "gratuito",
  "entrada libre": "gratuito",
  "lugares gratuitos": "gratuito",
  museo: "museo",
  museos: "museos",
  cultural: "cultural",
  hotel: "hotel",
  ...
  comer: "restaurante",
  comida: "comida",
  "plato tipico": "gastronomia",
  "platos tipicos": "gastronomia",
  "comida tipica": "gastronomia",
  degustar: "gastronomia",
  parque: "parque",
  parques: "parques",
  jardin: "parque",
  jardines: "parque",
  ...
  montaña: "natural",
  bosque: "natural",
  "atractivos naturales": "natural",
  senderismo: "senderismo",
  sendero: "senderismo",
  caminata: "senderismo",
  trekking: "senderismo",
  "rutas de senderismo": "senderismo",
  mirador: "mirador",
  vista: "panoramico",
  cerros: "mirador",
  ...
  "medios de transporte": "transporte",
  accesible: "accesible",
  ...
  niños: "familia",
  infantil: "familia",
  "recomendados para familias": "familia",
  monumento: "cultural",
  monumentos: "cultural",
  historico: "cultural",
  historicos: "cultural",
  ...
  "feria artesanal": "evento",
  "ferias artesanales": "evento",
  artesania: "evento",
  // INGLÉS
  free: "gratuito",
  "free entry": "gratuito",
  ...
  disability: "accesible",
  ruins: "arqueologico",
  archaeological: "arqueologico",
  family: "familia",
  kids: "familia",
  children: "familia",
  "recommended for families": "familia",
  monument: "cultural",
  monuments: "cultural",
  historical: "cultural",
  heritage: "cultural",
  ...
  "senza costo": "gratuito",
  museo: "museo",
  musei: "museos",
  hotel: "hotel",
  albergo: "hospedaje",
  alloggio: "hospedaje",
  "dove dormire": "hospedaje",
  ristorante: "restaurante",
  ristoranti: "restaurantes",
  mangiare: "restaurante",
  "piatto tipico": "gastronomia",
  ...
  fiume: "natural",
  bosco: "natural",
  "attrazioni naturali": "natural",
  sentiero: "senderismo",
  escursione: "senderismo",
  camminata: "senderismo",
  "percorsi di trekking": "senderismo",
  belvedere: "mirador",
  vista: "mirador",
  panoramico: "mirador",
  collina: "mirador",
  ...
  taxi: "transporte",
  treno: "transporte",
  "mezzi di trasporto": "transporte",
  accessibile: "accesible",
  "sedia a rotelle": "accesible",
  disabilità: "accesible",
  rovine: "arqueologico",
  archeologico: "arqueologico",
  scavi: "arqueologico",
  famiglia: "familia",
  bambini: "familia",
  "consigliato per famiglie": "familia",
  monumento: "cultural",
  monumenti: "cultural",
  storico: "cultural",
  patrimonio: "cultural",
  "punti di interesse": "cultural",
  "fiera artigianale": "evento",
  "fiere artigianali": "evento",
  artigianato: "evento",
};
...
// ============================================================
// DETECCIÓN DE IDIOMA Y TRADUCCIÓN DE RESULTADOS
// ============================================================

function detectarIdioma(texto) {
  if (!texto || texto.length < 2) return "es";
  const lower = texto.toLowerCase();

  // Palabras características en inglés
  ...
  for (const palabra of palabrasIngles) {
    if (lower.includes(palabra)) contIngles++;
  }
  for (const palabra of palabrasItaliano) {
    if (lower.includes(palabra)) contItaliano++;
  }

  if (contIngles > contItaliano && contIngles > 0) return "en";
  if (contItaliano > contIngles && contItaliano > 0) return "it";
  return "es";
}

// DICCIONARIOS DE TRADUCCIÓN DE CAMPOS
const TRADUCCIONESFIELDS = {
  es: {
    nombre: "Nombre",
    clase: "Clase",
    ...
    ubicacion: "Ubicación",
    horario: "Horario",
    tieneDescuento: "Tiene Descuento",
    requiereReserva: "Requiere Reserva",
    accesibilidad: "Accesibilidad",
    costoEntrada: "Costo Entrada",
    gratuito: "Gratuito",
    actividades: "Actividades",
    tipoEcosistema: "Tipo Ecosistema",
    tipoPatrimonio: "Tipo Patrimonio",
  },
  en: {
    nombre: "Name",
    clase: "Type",
    ...
    ubicacion: "Location",
    horario: "Hours",
    tieneDescuento: "Has Discount",
    requiereReserva: "Requires Reservation",
    accesibilidad: "Accessibility",
    costoEntrada: "Entrance Fee",
    gratuito: "Free",
    actividades: "Activities",
    tipoEcosistema: "Ecosystem Type",
    tipoPatrimonio: "Heritage Type",
  },
  it: {
    nombre: "Nome",
    clase: "Tipo",
    ...
    horario: "Orari",
    tieneDescuento: "Ha Sconto",
    requiereReserva: "Richiede Prenotazione",
    accesibilidad: "Accessibilità",
    costoEntrada: "Prezzo Ingresso",
    gratuito: "Gratuito",
...
// DICCIONARIOS DE VALORES DE CATEGORÍAS
const TRADUCCIONESCATEGORIAS = {
  es: {
    Atractivo_Cultural: "Atractivo Cultural",
    Atractivo_Natural: "Atractivo Natural",
    Atractivo_Recreativo: "Atractivo Recreativo",
    Atractivo_Arqueológico: "Atractivo Arqueológico",
    Hospedaje: "Hospedaje",
    Establecimiento_Gastronomico: "Establecimiento Gastronómico",
    Producto_Alimenticio: "Producto Alimenticio",
    Evento_Turístico: "Evento Turístico",
    Transporte: "Transporte",
  },
  en: {
    Atractivo_Cultural: "Cultural Attraction",
    Atractivo_Natural: "Natural Attraction",
    Atractivo_Recreativo: "Recreational Attraction",
    Atractivo_Arqueológico: "Archaeological Attraction",
    Hospedaje: "Lodging",
    Establecimiento_Gastronomico: "Restaurant",
    Producto_Alimenticio: "Food Product",
    Evento_Turístico: "Tourism Event",
    Transporte: "Transportation",
  },
  it: {
    Atractivo_Cultural: "Attrazione Culturale",
    Atractivo_Natural: "Attrazione Naturale",
    Atractivo_Recreativo: "Attrazione Ricreativa",
    Atractivo_Arqueológico: "Attrazione Archeologica",
    Hospedaje: "Alloggio",
    Establecimiento_Gastronomico: "Ristorante",
    Producto_Alimenticio: "Prodotto Alimentare",
    Evento_Turístico: "Evento Turistico",
    Transporte: "Trasporto",
  },
};

function traducirEntidad(entidad, idioma) {
  if (idioma === "es") return entidad; // Sin traducción para español

  const traducida = { ...entidad };
  const dictCat = TRADUCCIONESCATEGORIAS[idioma] || TRADUCCIONESCATEGORIAS.es;

  // Traducir clase
  if (traducida.clase) {
    traducida.clase =
      dictCat[traducida.clase.replace(/ /g, "_")] || traducida.clase;
  }

  // Traducir booleanos a texto
  if (traducida.gratuito !== null && traducida.gratuito !== undefined) {
    traducida.gratuito =
      idioma === "en"
        ? traducida.gratuito
          ? "Yes"
  ...
  if (
    traducida.accesibilidad !== null &&
    traducida.accesibilidad !== undefined
  ) {
    traducida.accesibilidad =
      idioma === "en"
        ? traducida.accesibilidad
          ? "Yes"
          : "No"
...
function buscarParquesEspecifico() {
  ...
  for (const st of todosLosIndividuos()) {
    ...
    if (!esParque) {
      ...
      if (
        clase.includes("Atractivo_Recreativo") &&
        (nombre.toLowerCase().includes("parque") ||
          nombre.toLowerCase().includes("park"))
      ) {
        esParque = true;
      }
    }
    ...
    for (const termino of intencion.terminos) {
      ...
      for (const palabra of palabrasQuery) {
        if (palabra === nt) puntaje += 15;
          ...
          palabra.length > 2 &&
          nt.length > 2 &&
          (palabra.includes(nt) || nt.includes(palabra))
        )
          puntaje += 10;
        else if (palabra.length > 3 && levenshtein(palabra, nt) <= 2)
          puntaje += 5;
      }
    }
...
function buscarPorClase(clase) {
  return individuosDe(clase).map((st) =>
    normalizarEntidad(st.subject.value, obtenerPropiedades(st.subject)),
  );
}
...
function obtenerPropiedades(uriNode) {
  ...
  statements.forEach((st) => {
    ...
    } else if (pred === OWL + "sameAs") {
      // CAPTURAR LINKS owl:sameAs (conexiones a DBpedia y Wikipedia)
      if (!props._sameAs) props._sameAs = [];
      props._sameAs.push(obj.value);
    } else {
      if (!props._objProps) props._objProps = {};
      if (!props._objProps[predName]) props._objProps[predName] = [];
      props._objProps[predName].push(
        obj.value.split("#").pop().replace(/_/g, " "),
      );
    }
  });
  return props;
...
function getProp(props, name) {
  return props[name]?.length ? props[name][0].value : null;
}
  ...
  for (const st of todosLosIndividuos()) {
    ...
    const camposNombre = [
      ...
      (() => {
        ...
        return i >= 0
          ? decodeURIComponent(uri.slice(i + 1)).replace(/_/g, " ")
          : null;
      })(),
    ].filter(Boolean);
...
// ============================================================
// FUNCIÓN PRINCIPAL buscar (CORREGIDA)
// ============================================================
...
async function buscar(q) {
  ...
  // DETECTAR IDIOMA DE LA BÚSQUEDA
  const idioma = detectarIdioma(query);
  console.log(
    `🌐 Idioma detectado: ${idioma === "en" ? "English" : idioma === "it" ? "Italian" : "Español"}`,
  );

  const traducida = traducirConsulta(query);
  const queryFinal = traducida !== query ? traducida : query;
  if (traducida !== query)
    console.log(`🔄 Traducida a categoría: "${traducida}"`);
  ...
  let resultados = [];
  const todosExactos = [
    ...new Map([...exactos, ...exactosOrig].map((e) => [e.nombre, e])).values(),
  ];
  const todosParciales = [
    ...new Map(
      [...parciales, ...parcialesOrig].map((e) => [e.nombre, e]),
    ).values(),
  ];

  if (todosExactos.length > 0) {
    console.log(`✅ Exactos por nombre: ${todosExactos.length}`);
    resultados = todosExactos;
  } else if (todosParciales.length > 0) {
    console.log(`✅ Parciales por nombre: ${todosParciales.length}`);
    resultados = todosParciales;
  } else {
    ...
    if (intencion) {
      ...
      if (porIntencion.length > 0) {
        ...
        resultados = porIntencion;
      }
    }

    if (resultados.length === 0) {
      console.log(`🔤 Texto libre`);
      resultados = buscarTextoLibre(queryFinal);
      console.log(`✅ Texto libre: ${resultados.length}`);
    }
  }

  // TRADUCIR RESULTADOS AL IDIOMA DETECTADO
  if (idioma !== "es") {
    console.log(
      `🌍 Traduciendo ${resultados.length} resultados a ${idioma === "en" ? "English" : "Italian"}...`,
    );
    resultados = resultados.map((r) => traducirEntidad(r, idioma));
  }

  // ENRIQUECER RESULTADOS CON DBPEDIA (silenciosamente - si falla, no afecta)
  try {
    console.log(
      `🌍 Enriqueciendo ${resultados.length} resultados con DBpedia...`,
    );
    const enriquecidos = await enriquecerListaConDBpedia(resultados);
    console.log(`✅ Enriquecimiento completado`);
    return enriquecidos;
  } catch (error) {
    console.log(`⚠️ Enriquecimiento DBpedia falló: ${error.message}`);
    console.log(
      `✅ Devolviendo ${resultados.length} resultados sin enriquecer`,
    );
    return resultados;
  }
}
...
function buscarPorPrefijo(prefijo) {
  if (!loaded) cargarOntologia();
  if (!prefijo || prefijo.trim().length < 2) return Promise.resolve([]);
  return buscar(prefijo.trim()).then((r) => r.slice(0, 20));
}
...
function serializarAOWL(entidades, termino) {
  ...
  for (let i = 0; i < entidades.length; i++) {
    ...
    const sameAsLinks = (list) => {
      if (list?.length) {
        list.forEach((url) => {
          owl += `\n    <owl:sameAs rdf:resource="${escapeXml(url)}"/>`;
        });
      }
    };
    ...
    sameAsLinks(e.sameAs);

    // Serializar datos enriquecidos de DBpedia (si existen)
    if (e.dbpedia) {
      if (e.dbpedia.dbpediaLabel) a("dbpediaLabel", e.dbpedia.dbpediaLabel);
      if (e.dbpedia.dbpediaAbstract)
        a("dbpediaAbstract", e.dbpedia.dbpediaAbstract);
      if (e.dbpedia.dbpediaThumbnail)
        a("dbpediaThumbnail", e.dbpedia.dbpediaThumbnail);
      if (e.dbpedia.dbpediaLat !== null && e.dbpedia.dbpediaLat !== undefined)
        an("dbpediaLat", e.dbpedia.dbpediaLat);
      if (e.dbpedia.dbpediaLong !== null && e.dbpedia.dbpediaLong !== undefined)
        an("dbpediaLong", e.dbpedia.dbpediaLong);
      if (e.dbpedia.dbpediaWikiPage)
        a("dbpediaWikiPage", e.dbpedia.dbpediaWikiPage);
    }
...
// Inicializar la ontología al cargar el módulo
cargarOntologia();
